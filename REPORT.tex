...
The core parallel processing is implemented using Python's concurrent.futures module:

\begin{lstlisting}[language=Python, caption={BatchProcessor Implementation}]
...
\begin{lstlisting}[language=Python, caption={SQS Consumer Implementation}]
class ParallelSQSConsumer:
    def consume_batch(self, max_messages=10):
...
\begin{lstlisting}[language=Python, caption={External API Implementation}]
class WeatherService:
    BASE_URL = "https://api.openweathermap.org/data/2.5/weather"
    
    def get_weather(self, city):
        response = requests.get(
            f"{self.BASE_URL}?q={city}"
...
class CurrencyExchangeService:
    ...
    def get_exchange_rate(self, from_curr, 
                          to_curr, amount):
        return {
...
cd dist && zip -r ../lambda-package.zip .

# Deploy Lambda
aws lambda update-function-code \
  --function-name HarshScalable-inventory-api \
  --zip-file fileb://lambda-package.zip

# Deploy frontend to S3
aws s3 sync frontend/dist \
  s3://inventory-frontend-prod-1773829508 \
  --delete
...
\begin{lstlisting}[language=Python, caption={Async Batch Processing}]
